...
describe('logic', () => {
        ...
        it('should succeed on correct credentials', () =>
            logic.authenticateUser(email, password)
                .then(({ token, idUser }) => {
                    ...
                    const decoded = jwt.verify(token, 'asdfasfdsafd');
                    expect(decoded.idUser).toEqual(idUser)
                    //console.log(decoded)
                })
        )
    })
    ...
    describe('retrieve user', () => {
        ...
        beforeEach(() =>
            users.add({ name, surname, email, password })
                .then(() => users.findEmail(email))
                .then(user => {
                    const idUser = user._id.toString()
                    const token = jwt.sign({ idUser }, 'asdfasfdsafd');
                    _id = idUser
                    _token = token
                })
        ...
        it('should succeed on correct credentials', () =>
            logic.retrieveUser(_id, _token)
                .then(user => {
                    expect(user._id.toString()).toBe(_id)
    ...
    describe('update user', () => {
        ...
        it('should succeed on correct credentials', () =>
            users.updateUser(_id, data)
                .then(() => users.findId(_id))
                .then(user => {
                    expect(user._id.toString()).toBe(_id)
